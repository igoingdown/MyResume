...
\cventry{\small{2021.07-至今}}{\textcolor[RGB]{64,127,191}{字节跳动}}{中国电商}{后端开发工程师}{获 spotbonus 3 次，表彰\textbf{业务和稳定性}方面的突出贡献}{
    \begin{itemize}
        \item \textcolor[RGB]{64,127,191}{商城增长方向负责人}
        \newline \textbf{业务建设}：主导签到、报销机等核心玩法从0到1建设，玩法在商城新入口\textbf{GMV渗透率超7+\%}；在红包雨发券引入算法模型，GMV转化率优于人工配置，\textbf{玩法GMV\_ROI长期显著大于1}；落地红包雨玩法商业化机制， 实现\textbf{单月收入超1000万人民币}，成功达成玩法收支平衡。
        \newline \textbf{技术建设}：抽象并落地通用钱包系统，统一现金发放与管理，\textbf{支撑3+核心玩法快速上线}。梳理玩法通用能力，
        建设组件化SDK，已沉淀\textbf{Feed流、任务列表、权益列表}等可复用组件，提升开发效率
        \newline \textbf{效能与稳定性}：规划并落地通用玩法配置平台，支持\textbf{3个玩法实现RD零成本配置上线}，配置效率大幅提升；沉淀全过程SOP文档10+，提升开发、测试与Oncall效率；为各玩法定制稳定性方案，沉淀稳定性方案模板，保障\textbf{5+玩法集中上线0故障}，模板使用 UV \textbf{10+}
        ...
        \newline \textbf{存量问题召回与治理}：推动 B 端和 C 端导购链路接入 panic 检测，召回并解决 3+ panic 问题；
        完成 B\&C 端链路强弱依赖梳理，召回 P0 问题 \textbf{10+}，落地改进 \textbf{4} 个；
        分析历史压测链路，定位压测缓存穿透率低问题，建设通用压测缓存穿透 sdk，缓存穿透率\textbf{0.1\%\->50\%}，压测精度提升
        \newline \textbf{机制强化}：建设稳定性监控大盘，覆盖全链路 psm 和基础组件，显著提升了问题排查效率；建设日志降采样大盘，规划并落地日志清理专项，C 端日志降采样率降至\textbf{0}；
        开发爬虫脚本抓取全量报警规则数据，review 并修改报警规则；与 QA 建设定期报警治理机制，消防群报警数显著下降 \textbf{80+\%}，报警响应率长期保持 \textbf{100\%}
        \newline \textbf{效能建设}：开发稳定性工具脚本仓库，支持按服务树节点拉取全量faas 脚本及触发器配置、全量 psm 报警配置，实现消费限速巡检报警 review 提效
